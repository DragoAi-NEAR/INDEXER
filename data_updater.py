...
def update_data():
    ...
    for token in token_ids:
        ...
        logger.info(f"Processing token {token_id}")
        try:
            token_price = fetch_token_price(token_id)
            token_overview = get_token_overview(token_id)
            ...
            if token_overview:
                ...
                holders = fetch_token_holders(token_id)
                if holders:
                    for holder in holders:
                        account = holder['account']
                        amount = int(holder['amount']) / (10 ** token_overview["decimals"])
                        percentage = (amount / token_overview['total_supply']) * 100
                        insert_holders(token_id, token_db_id, account, amount, percentage)
            else:
                logger.warning(f"Failed to fetch token overview for {token_id}")
